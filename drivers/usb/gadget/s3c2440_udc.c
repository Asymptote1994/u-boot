...
static inline void udc_writeb(u32 base, u32 value, u32 reg)
{
...
static inline void s3c2410_udc_clear_ep0_opr(u32 base)
{
...
static inline void s3c2410_udc_clear_ep0_sst(u32 base)
{
	udc_writeb(base, S3C2410_UDC_INDEX_EP0, S3C2410_UDC_INDEX_REG);
	writeb(0x00, base + S3C2410_UDC_EP0_CSR_REG);
}

static inline void s3c2410_udc_clear_ep0_se(u32 base)
{
	udc_writeb(base, S3C2410_UDC_INDEX_EP0, S3C2410_UDC_INDEX_REG);
	udc_writeb(base, S3C2410_UDC_EP0_CSR_SSE, S3C2410_UDC_EP0_CSR_REG);
}

static inline void s3c2410_udc_set_ep0_ipr(u32 base)
{
	udc_writeb(base, S3C2410_UDC_INDEX_EP0, S3C2410_UDC_INDEX_REG);
	udc_writeb(base, S3C2410_UDC_EP0_CSR_IPKRDY, S3C2410_UDC_EP0_CSR_REG);
}

static inline void s3c2410_udc_set_ep0_de(u32 base)
{
	udc_writeb(base, S3C2410_UDC_INDEX_EP0, S3C2410_UDC_INDEX_REG);
	udc_writeb(base, S3C2410_UDC_EP0_CSR_DE, S3C2410_UDC_EP0_CSR_REG);
}

inline void s3c2410_udc_set_ep0_ss(u32 b)
{
	udc_writeb(b, S3C2410_UDC_INDEX_EP0, S3C2410_UDC_INDEX_REG);
	udc_writeb(b, S3C2410_UDC_EP0_CSR_SENDSTL, S3C2410_UDC_EP0_CSR_REG);
}

static inline void s3c2410_udc_set_ep0_de_out(u32 base)
{
	udc_writeb(base, S3C2410_UDC_INDEX_EP0, S3C2410_UDC_INDEX_REG);
...
static inline void s3c2410_udc_set_ep0_de_in(u32 base)
{
	udc_writeb(base, S3C2410_UDC_INDEX_EP0, S3C2410_UDC_INDEX_REG);
		...
		struct s3c2410_request *req)
{
	...
	if (idx == 0)
		debug("Written ep%d %d.%d of %d b [last %d,z %d]\n",
...
static int s3c2410_udc_set_descriptor(void);
/*---------------------------------------------------------------------------*/
					...
					u32 ep0csr)
{
	...
	if (len != sizeof(*crq)) {
		debug("setup begin: fifo READ ERROR"
	...
	debug("bRequest = %d bRequestType %d wLength = %d\n",
	...
	if (crq->bRequestType & USB_DIR_IN) {
		debug("%s(): EP0_IN_DATA_PHASE ...\n", __func__);
		dev->ep0state = EP0_IN_DATA_PHASE;
	} else {
		debug("%s(): EP0_OUT_DATA_PHASE ...\n", __func__);
		dev->ep0state = EP0_OUT_DATA_PHASE;
	}

	s3c2410_udc_set_descriptor();
	...
	if (ret < 0) {
		if (dev->req_config) {
			debug("config change %02x fail %d?\n",
				crq->bRequest, ret);
			return;
		}
...
static void s3c2410_udc_handle_ep0(struct s3c2410_udc *dev)
{
	...
	debug("ep0csr %x ep0state %s\n",
		ep0csr, ep0states[dev->ep0state]);

	/* clear stall status */
	if (ep0csr & S3C2410_UDC_EP0_CSR_SENTSTL) {
		s3c2410_udc_nuke(dev, ep, -EPIPE);
		debug("... clear SENT_STALL ...\n");
...
static irqreturn_t s3c2410_udc_irq(int dummy, void *_dev)
{
	...
	// debug("usbs=%02x, usbds=%02x, pwr=%02x ep0csr=%02x\n",
	...
	if (usbd_status & S3C2410_UDC_INT_EP0) {
		debug("func: %s(), line: %d   USB ep0 irq\n", __func__, __LINE__);
	...
	for (i = 1; i < S3C2410_ENDPOINTS; i++) {
		...
		if (usbd_status & tmp) {
			...
			debug("func: %s(), line: %d   USB ep%d irq\n", __func__, __LINE__, i);
				 ...
				 const struct usb_endpoint_descriptor *desc)
{
	...
	debug("enable %s(%d) ep%x%s-blk max %02x\n",
		 _ep->name, ep->num, tmp,
...
static void s3c2410_udc_command(enum s3c2410_udc_cmd_e cmd)
{
	...
	ret = gpio_request_by_name(g_dev, "pullup-gpio", 0, &pullup_pin, GPIOD_IS_OUT);
	if (ret) {
		printf("%s(): gpio_request_by_name failed!\n", __func__);
		return ret;
	}
	
	if (dm_gpio_is_valid(&pullup_pin))
		dm_gpio_set_value(&pullup_pin, value);

	printf("%s(): dm_gpio_set_value success!\n", __func__);

	dm_gpio_free(g_dev, &pullup_pin);
...
static void s3c2410_udc_enable(struct s3c2410_udc *dev)
{
	...
	debug("s3c2410_udc_enable called\n");
...
static int s3c2410_udc_set_descriptor(void)
{
	...
	struct usb_ep *ep = &memory.ep[0].ep;

	debug("func: %s(), line: %d\n", __func__, __LINE__);

	if (flag == 0) {
		req = s3c2410_udc_alloc_request(ep, GFP_KERNEL);
		flag = 1;
	}
	...
	memcpy(req->buf, &device_desc, sizeof(device_desc));

	// s3c2410_udc_queue(ep, req, GFP_KERNEL);
	s3c2410_udc_write_fifo(to_s3c2410_ep(ep), to_s3c2410_req(req));
	
	return 0;
}

int s3c2410_udc_handle_interrupt(void)
{
	// u32 intr_status = readl(&reg->gintsts);
	// u32 gintmsk = readl(&reg->gintmsk);

	// if (intr_status & gintmsk)
		return s3c2410_udc_irq(1, (void *)the_controller);
...
{
	return s3c2410_udc_handle_interrupt();
}

/*
 *	probe - binds to the platform device
 */
static int s3c2410_udc_probe(struct udevice *dev)
{
...
#define	UPLLCON			(*(volatile unsigned long *)0x4c000008)
	...
	debug("got and enabled clocks\n");

	// if (strncmp(pdev->name, "s3c2440", 7) == 0) {
		debug("S3C2440: increasing FIFO to 128 bytes\n");
 ...
 *	s3c2410_udc_remove
 */
static int s3c2410_udc_remove(struct udevice *dev)
{
...
static int s3c_udc_ofdata_to_platdata(struct udevice *dev)
{
	return 0;
}

static const struct udevice_id s3c_udc_ids[] = {
	{ .compatible = "samsung,s3c2410-usbgadget" },
	{},
};

U_BOOT_DRIVER(s3c2440_udc) = {
	.name	= "s3c24x0-usbgadget",
	.id	= UCLASS_USB_GADGET_GENERIC,
	.of_match = s3c_udc_ids,
	.ofdata_to_platdata = s3c_udc_ofdata_to_platdata,
	.probe = s3c2410_udc_probe,
	.remove = s3c2410_udc_remove,
	.platdata_auto_alloc_size = sizeof(struct s3c2410_udc),
	.priv_auto_alloc_size = sizeof(struct s3c2410_udc),
};
